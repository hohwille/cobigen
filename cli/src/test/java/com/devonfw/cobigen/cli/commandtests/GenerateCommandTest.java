 ...
 */
public class GenerateCommandTest extends AbstractCommandTest {
  ...
  public void generateFromEntityTest() {
    ...
    TestMavenProject project = createProject("genFromEntity");
    execute("generate", project.getEntity().getAbsolutePath(), "--increments", "springdata-repository");
    assertThat(project.getRepository()).exists().hasSameContentAs(getExpectedFile("TestEntityRepository.java"));
  }
  ...
  public void generateNonJavaFilesFromJavaInputTest() {
    ...
    TestMavenProject project = createProject("genNonJavaFilesFromEntity");
    execute("generate", project.getEntity().getAbsolutePath(), "--increments", "16");
  }
  //
  // /**
  ...
  // File openApiFile = new File(testFileRootPath + "openAPI copy.yml");
